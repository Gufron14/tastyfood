  ...
  <div class="d-flex justify-content-center mt-4">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb text-center">
            <li class="breadcrumb-item"><a href="{{ route('news') }}" style="color: #4c00ff">Home</a></li>
            <li class="breadcrumb-item active" aria-current="page">{{ $posts->title }}</li>
        </ol>
    </nav>
</div>

    <div class="row justify-content-center gap-2 mb-5">
        <div class="col-3" style="height: 600px">
            @foreach ($thumbnails as $thumbnail)
                <div class="view-thumbnail mb-5">
                    <img src="{{ asset('thumbnail/' . $thumbnail->thumbnail) }}" alt="" class="w-100 h-100 object-fit-cover rounded">
                </div>
            @endforeach
        </div>
        <div class="col-5">
            <div class=" mb-5" style="width: 500px">
                <div class="d-flex justify-content-center">
                    <div class="news-thumbnail mb-5 shadow rounded">
                        <img src="{{ asset('thumbnail/' . $posts->thumbnail) }}" alt="{{ $posts->thumbnail }}" class="w-100 h-100 rounded object-fit-cover">
                    </div>
                </div>
        ...
        <div class="col-3" style="height: 600px">
            @foreach ($galleries as $gallery)
                <div class="gallery mb-3">
                    <img src="{{ asset('image/' .$gallery->image) }}" alt="" class="w-100 h-100 object-fit-cover rounded">
                </div>
            @endforeach
        </div>
    </div>
